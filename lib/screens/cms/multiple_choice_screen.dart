...
class MultipleChoiceScreen extends StatefulWidget {
  ...
  const MultipleChoiceScreen({Key? key, required this.plantId}) : super(key: key);
...
class _MultipleChoiceScreenState extends State<MultipleChoiceScreen> {
  ...
  void initState() {
    ...
    if (widget.plantId.isEmpty) {
      ...
      // Dummy data untuk Multiple Choice
      questions = List.generate(
        5,
        (index) => MCQuestion(
          kode: "MCQ-${index + 1}",
          statement: "Contoh pertanyaan pilihan ganda nomor ${index + 1}",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("CMS - Multiple Choice (${widget.plantId})"),
